...
def AudioStandarize(audio_file, sr=320000, device=None, high_pass=0):
  if not device:
...
class SilicBat:
  ...
  def spectrogram(self, audiodata, spect_type='linear', rainbow_bands=8):
  ...
  def tfr(self, targetfilepath=None, spect_type='linear', rainbow_bands=8, start=0, stop=None):
    ...
    if spect_type == 'rainbow' and rainbow_bands == 8:
  ...
  def detect(self, weights, step=1000, conf_thres=0.1, imgsz=640, targetfilepath=None, iou_thres=0.25, soundclasses=None):
    ...
    labels = [['file', "time_begin", "time_end", "freq_low", "freq_high", 'classid', "score"]]
      ...
      for det in pred:    # detections per image
        ...
        if len(det):
          ...
          for *xyxy, conf, cls in reversed(det):
            ...
            ts, te, fl, fh = ttff
            labels.append([path, round(time_start+ts), round(time_start+te), fl, fh, self.names[int(cls)], round(float(conf),3)])
...
def clean_multi_boxes(labels, threshold_iou=0.25, threshold_iratio=0.75):
